...
public:
  ...
  bool listen(const char *host, int port, int socket_flags = 0, const std::function<void()>& MainProcess = nullptr);
...
private:
  ...
  bool listen_internal(const std::function<void()>& MainProcess = nullptr);
...
inline bool Server::listen(const char *host, int port, int socket_flags, const std::function<void()>& MainProcess) {
  return bind_to_port(host, port, socket_flags) && listen_internal(MainProcess);
}
...
inline bool Server::listen_internal(const std::function<void()>& MainProcess) {
  auto ret = true;
  is_running_ = true;
